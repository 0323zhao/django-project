...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        # Linux
        cmd = 'cd /d D:\BYSJ\SPIDER && scrapy crawl ' + tableName + 'Spider -a databaseName=djangop8gh5erc'
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=djangop8gh5erc"
        os.system(cmd)
...
def schemaName_mysqldump(request):
    if request.method in ["POST", "GET"]:
        ...
        filePath = os.path.join(os.getcwd(), "templates/upload", "backup.sql")
        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 djangop8gh5erc > " + filePath
        os.system(cmd)
...
def schemaName_wordCloud(request, tableName, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        allModels = apps.get_app_config('main').get_models()
        where = {}
        ...
        for item in msg['data']:
            city_name = item[columnName]
            if city_name is None:
                continue
            total = item["total"]
            # 使用jieba提取关键词
            # 对于城市名称，我们只提取一个最重要的关键词
            keywords = jieba.analyse.extract_tags(city_name, topK=1)
            keyword = keywords[0] if keywords else city_name
            # 累加相同关键词的total值
        ...
        # 构建结果数组
        msg['data'] = [{columnName: keyword, "total": total} for keyword, total in sorted_keywords if keyword is not None and 'None'!=keyword]
        return JsonResponse(msg)
