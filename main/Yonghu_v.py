    ...
    if request.method in ["POST", "GET"]:
        ...
        if __sfsh__ == '是':
            if datas[0].get('sfsh') != '是':
                ...
                return JsonResponse(msg, encoder=CustomJsonEncoder)
        #判断验证码是否正确
        smsregistercode_result = smsregistercode.objects.filter(mobile=mobile, code=smscode,
                                                            role="用户").order_by('-addtime').first()
        if smsregistercode_result is None:
            msg['code'] = password_error_code
            msg['msg'] = '验证码不正确'
            return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def yonghu_sendsms(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        code = ''.join(code)
        config_result = config.objects.filter(name="aliyun").first().value
