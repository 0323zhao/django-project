	...
	<div class="main-content" :style='{"padding":" 30px","background":"#DCEAF7","height":"100vh"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"borderRadius":"24px","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"20px 10px 0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0 0 30px 0","borderRadius":"24px","flexWrap":"wrap","borderWidth":"0","background":"#F0F5FB","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}'>
					<el-button :style='{"border":"1px solid #B2F9F0","cursor":"pointer","padding":"0px 24px","boxShadow":"inset 0px -6px 6px 1px #3EE3F9","color":"#fff","minWidth":"120px","outline":"none","borderRadius":"24px","background":" linear-gradient( 135deg, #087FF3 0%, #32B2F3 100%)","width":"110px","fontSize":"16px","fontWeight":"bold","height":"41px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"padding":"20px ","boxShadow":"none","margin":"0","borderRadius":"24px 24px 0 0","flexWrap":"wrap","background":"#F0F5FB","display":"flex","width":"100%","justifyContent":"0"}'>
					...
					<el-button class="add" v-if="isAuth('discussblogcomment','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('discussblogcomment','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"calc(100%) ","padding":"0 30px 30px 30px","margin":"0px 0 0 0","borderRadius":"0 0 24px 24px","background":"#F0F5FB","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussblogcomment','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('discussblogcomment','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('blogcomment','删除评论')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussblogcomment',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussblogcomment/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'blogcomment/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'blogcomment/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
