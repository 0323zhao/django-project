		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"borderRadius":"24px","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"20px ","boxShadow":"none","margin":"0","borderRadius":"24px 24px 0 0","flexWrap":"wrap","background":"#F0F5FB","display":"flex","width":"100%","justifyContent":"0"}'>
					...
					<el-button class="add" v-if="isAuth('discussbloginfo','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('discussbloginfo','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"calc(100%) ","padding":"0 30px 30px 30px","margin":"0px 0 0 0","borderRadius":"0 0 24px 24px","background":"#F0F5FB","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussbloginfo','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('discussbloginfo','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('bloginfo','删除评论')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussbloginfo',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussbloginfo/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'bloginfo/info/' + refid,
								method: "get",
							}).then(res=>{
