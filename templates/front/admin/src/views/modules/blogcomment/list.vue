...
<template>
	<div class="main-content" :style='{"padding":" 30px","background":"#DCEAF7","height":"100vh"}'>
			...
			<el-form class="center-form-pv" :style='{"borderRadius":"24px","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0 0 30px 0","borderRadius":"24px","flexWrap":"wrap","borderWidth":"0","background":"#F0F5FB","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">评论人</label>
						<el-input v-model="searchForm.pluser" placeholder="评论人" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">发布地</label>
						<el-input v-model="searchForm.fbplace" placeholder="发布地" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px ","boxShadow":"none","margin":"0","borderRadius":"24px 24px 0 0","flexWrap":"wrap","background":"#F0F5FB","display":"flex","width":"100%","justifyContent":"0"}'>
					<el-button class="add" v-if="isAuth('blogcomment','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('blogcomment','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('blogcomment','爬取数据')" type="success" @click="spider()">
			...
			<div :style='{"width":"calc(100%) ","padding":"0 30px 30px 30px","margin":"0px 0 0 0","borderRadius":"0 0 24px 24px","background":"#F0F5FB","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('blogcomment','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('blogcomment','评论分析')" @click="pinglunfenxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								评论分析
							</el-button>
							<el-button class="edit" v-if=" isAuth('blogcomment','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('blogcomment','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('blogcomment','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<pinglunfenxi-cross-add-or-update v-if="pinglunfenxiCrossAddOrUpdateFlag" :parent="this" ref="pinglunfenxiCrossaddOrUpdate"></pinglunfenxi-cross-add-or-update>



		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('blogcomment','发布地')">
					<div id="fbplaceChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	import pinglunfenxiCrossAddOrUpdate from "../pinglunfenxi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				pinglunfenxiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			pinglunfenxiCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('blogcomment','发布地')) {
			...
			pinglunfenxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.pinglunfenxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','blogcomment');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.pinglunfenxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.pinglunfenxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var fbplaceChart1 = echarts.init(document.getElementById("fbplaceChart1"),'macarons');
					let params = {}
					this.$http({
						url: "blogcomment/group/fbplace",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							fbplaceChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								fbplaceChart1.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.pluser!='' && this.searchForm.pluser!=undefined){
					params['pluser'] = '%' + this.searchForm.pluser + '%'
				}
				if(this.searchForm.fbplace!='' && this.searchForm.fbplace!=undefined){
					params['fbplace'] = '%' + this.searchForm.fbplace + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "blogcomment/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussblogcomment',query:{refid:id}});
			},
			// 删除
