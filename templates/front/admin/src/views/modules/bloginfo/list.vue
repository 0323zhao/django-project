...
<template>
	<div class="main-content" :style='{"padding":" 30px","background":"#DCEAF7","height":"100vh"}'>
		<!-- 列表页 -->
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"borderRadius":"24px","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0 0 30px 0","borderRadius":"24px","flexWrap":"wrap","borderWidth":"0","background":"#F0F5FB","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">博主</label>
						<el-input v-model="searchForm.screenname" placeholder="博主" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">发布时间</label>
						<el-input v-model="searchForm.fbtime" placeholder="发布时间" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">媒体</label>
						<el-input v-model="searchForm.medias" placeholder="媒体" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px ","boxShadow":"none","margin":"0","borderRadius":"24px 24px 0 0","flexWrap":"wrap","background":"#F0F5FB","display":"flex","width":"100%","justifyContent":"0"}'>
					<el-button class="add" v-if="isAuth('bloginfo','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('bloginfo','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('bloginfo','爬取数据')" type="success" @click="spider()">
			...
			<div :style='{"width":"calc(100%) ","padding":"0 30px 30px 30px","margin":"0px 0 0 0","borderRadius":"0 0 24px 24px","background":"#F0F5FB","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('bloginfo','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('bloginfo','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('bloginfo','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('bloginfo','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<template v-if="statType">
			<div class="statShow5">
				<div class="echarts1" v-if="isAuth('bloginfo','热门博主')">
					<div id="screennameChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('bloginfo','媒体占比')">
					<div id="mediasChart2" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts3" v-if="isAuth('bloginfo','博主评论数')">
					<div id="commentscountChart3" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts4" v-if="isAuth('bloginfo','博主转发数')">
					<div id="repostscountChart4" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts5" v-if="isAuth('bloginfo','博主点赞数')">
					<div id="attitudescountChart5" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			statChartClick() {
				if(this.isAuth('bloginfo','热门博主')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('bloginfo','媒体占比')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('bloginfo','博主评论数')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('bloginfo','博主转发数')) {
					this.chartDialogShow4()
				}
				if(this.isAuth('bloginfo','博主点赞数')) {
					this.chartDialogShow5()
				}
			},
			queryChange(arr){
				for(let x in arr) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "wordCloud/bloginfo/screenname",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							var option = {};
							this.wordclouds(pArray,'screennameChart1');
						}
					})
				})
			},
			...
			chartDialog3() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "bloginfo/value/screenname/commentscount",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialog4() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "bloginfo/value/screenname/repostscount",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialog5() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "bloginfo/value/screenname/attitudescount",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			getDataList() {
				...
				if(this.searchForm.screenname!='' && this.searchForm.screenname!=undefined){
					params['screenname'] = '%' + this.searchForm.screenname + '%'
				}
				if(this.searchForm.fbtime!='' && this.searchForm.fbtime!=undefined){
					params['fbtime'] = '%' + this.searchForm.fbtime + '%'
				}
				if(this.searchForm.medias!='' && this.searchForm.medias!=undefined){
					params['medias'] = '%' + this.searchForm.medias + '%'
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussbloginfo',query:{refid:id}});
			},
			// 删除
...
<style lang="scss" scoped>
	...
	.main-content .el-pagination /deep/ .el-pagination__jump .el-input .el-input__inner {
		...
		line-height: 40px;
		border-radius: 8px;
		box-shadow: inset 0px 3px 6px 1px #216CF6;
		outline: 0;
		background: #fff;
		width: 100%;
		text-align: center;
