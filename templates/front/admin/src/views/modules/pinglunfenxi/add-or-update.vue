	...
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-else-if="ruleForm.plcontent" label="评论内容" prop="plcontent"  class="textBox">
				...
				<el-form-item class="input" v-if="type!='info'" label="情感分析" prop="qingganfenxi" >
					<el-input v-model="ruleForm.qingganfenxi" placeholder="情感分析" :readonly="ro.qingganfenxi"></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.qingganfenxi" label="情感分析" prop="qingganfenxi" >
					<el-input v-model="ruleForm.qingganfenxi" placeholder="情感分析" readonly></el-input>
				</el-form-item>
		...
		methods: {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='plcontent'){
							...
							this.sentimentChange(obj[o])
							this.ro.plcontent = true;
							continue;
						}
						if(o=='qingganfenxi'){
							this.ruleForm.qingganfenxi = obj[o];
							this.ro.qingganfenxi = true;
							continue;
						}
					}
				}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `pinglunfenxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.pinglunfenxiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			sentimentChange(e){
				...
				this.sentimentTimer = setTimeout(()=>{
					this.$http({
						url: "pinglunfenxi/baidu/sa",
						method: "get", 
						params: {
							text: e
						}
					}).then(rs=>{
						this.ruleForm.qingganfenxi = '您的情感偏' + rs.data.data.sentiment + (rs.data.data.sentiment=='正向'?'😀':(rs.data.data.sentiment=='中性'?'😐':'☹️'))
					})
				},1500)
			},
		}
