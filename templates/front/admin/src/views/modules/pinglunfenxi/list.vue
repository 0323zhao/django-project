...
<template>
	<div class="main-content" :style='{"padding":" 30px","background":"#DCEAF7","height":"100vh"}'>
			...
			<el-form class="center-form-pv" :style='{"borderRadius":"24px","width":"100%","margin":"0"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0","boxShadow":"nnoe","borderColor":"#00000050","margin":"0 0 30px 0","borderRadius":"24px","flexWrap":"wrap","borderWidth":"0","background":"#F0F5FB","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"16px","fontWeight":"500","height":"41px"}' class="item-label">情感分析</label>
						<el-input v-model="searchForm.qingganfenxi" placeholder="情感分析" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px ","boxShadow":"none","margin":"0","borderRadius":"24px 24px 0 0","flexWrap":"wrap","background":"#F0F5FB","display":"flex","width":"100%","justifyContent":"0"}'>
					<el-button class="add" v-if="isAuth('pinglunfenxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#883434","display":"none","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('pinglunfenxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
				...
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true'
						...
						<template slot-scope="scope">
							{{scope.row.qingganfenxi}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('pinglunfenxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('pinglunfenxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('pinglunfenxi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('pinglunfenxi','分析占比')">
					<div id="qingganfenxiChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			statChartClick() {
				if(this.isAuth('pinglunfenxi','分析占比')) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var qingganfenxiChart1 = echarts.init(document.getElementById("qingganfenxiChart1"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							qingganfenxiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								qingganfenxiChart1.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.qingganfenxi!='' && this.searchForm.qingganfenxi!=undefined){
					params['qingganfenxi'] = '%' + this.searchForm.qingganfenxi + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "pinglunfenxi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
