<template>
	<div class="home-content" :style="{'backgroundImage': indexBgUrl?`url(${$base.url + indexBgUrl})`:''}">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('bloginfo','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-xiaoliang1"></span>
				</div>
				<div class="right">
					<div class="num">{{bloginfoCount}}</div>
					<div class="name">博文信息总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('blogcomment','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-xiaoliang1"></span>
				</div>
				<div class="right">
					<div class="num">{{blogcommentCount}}</div>
					<div class="name">博文评论总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('pinglunfenxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-xiaoliang1"></span>
				</div>
				<div class="right">
					<div class="num">{{pinglunfenxiCount}}</div>
					<div class="name">评论分析总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 7 -->
		<div class="type7">
			<div class="echarts1 animate__animated" v-if="isAuth('bloginfo','首页统计',2)">
				<div id="bloginfoChart1" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts2 animate__animated" v-if="isAuth('bloginfo','首页统计',2)">
				<div id="bloginfoChart2" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts3 animate__animated" v-if="isAuth('bloginfo','首页统计',2)">
				<div id="bloginfoChart3" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts4 animate__animated" v-if="isAuth('bloginfo','首页统计',2)">
				<div id="bloginfoChart4" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts5 animate__animated" v-if="isAuth('bloginfo','首页统计',2)">
				<div id="bloginfoChart5" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts6 animate__animated" v-if="isAuth('blogcomment','首页统计',2)">
				<div id="blogcommentChart1" style="width: 100%;height: 100%"></div>
			</div>
			<div class="echarts7 animate__animated" v-if="isAuth('pinglunfenxi','首页统计',2)">
				<div id="pinglunfenxiChart1" style="width: 100%;height: 100%"></div>
			</div>
		</div>
		<!-- echarts -->
	</div>
...
export default {
	...
	mounted(){
		this.init();
		this.getbloginfoCount();
		if(this.isAuth('bloginfo','首页统计',2)){
			this.bloginfoChat1();
		}
		if(this.isAuth('bloginfo','首页统计',2)){
			this.bloginfoChat2();
		...
		if(this.isAuth('blogcomment','首页统计',2)){
			this.blogcommentChat1();
		}
		this.getpinglunfenxiCount();
		if(this.isAuth('pinglunfenxi','首页统计',2)){
			this.pinglunfenxiChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'bloginfoChart1',css:'animate__'},
				{id:'bloginfoChart2',css:'animate__'},
				{id:'bloginfoChart3',css:'animate__'},
				{id:'bloginfoChart4',css:'animate__'},
				{id:'bloginfoChart5',css:'animate__'},
				{id:'blogcommentChart1',css:'animate__'},
				{id:'pinglunfenxiChart1',css:'animate__'},
			]
		...
		getbloginfoCount() {
			this.$http({
				...
				if (data && data.code == 0) {
					this.bloginfoCount = data.data
				}
			})
		},
		bloginfoChat1(e=null) {
			this.$nextTick(()=>{
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						var option = {};
						this.wordclouds(pArray,'bloginfoChart1');
		...
		bloginfoChat2(e=null) {
			this.$nextTick(()=>{

				var bloginfoChart2 = echarts.init(document.getElementById("bloginfoChart2"),'macarons');
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						bloginfoChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							bloginfoChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		bloginfoChat3(e=null) {
			this.$nextTick(()=>{

				var bloginfoChart3 = echarts.init(document.getElementById("bloginfoChart3"),'macarons');
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						bloginfoChart3.setOption(option);
						//根据窗口的大小变动图表
						window.onresize = function() {
							bloginfoChart3.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		bloginfoChat4(e=null) {
			this.$nextTick(()=>{

				var bloginfoChart4 = echarts.init(document.getElementById("bloginfoChart4"),'macarons');
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						bloginfoChart4.setOption(option);

						//根据窗口的大小变动图表
						window.onresize = function() {
							bloginfoChart4.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		bloginfoChat5(e=null) {
			this.$nextTick(()=>{

				var bloginfoChart5 = echarts.init(document.getElementById("bloginfoChart5"),'macarons');
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						bloginfoChart5.setOption(option);
						//根据窗口的大小变动图表
						window.onresize = function() {
							bloginfoChart5.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getblogcommentCount() {
			this.$http({
				url: `blogcomment/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.blogcommentCount = data.data
				}
			})
		},
		blogcommentChat1(e=null) {
			this.$nextTick(()=>{
				var blogcommentChart1 = echarts.init(document.getElementById("blogcommentChart1"),'macarons');
				let params = {
				}
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						blogcommentChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							blogcommentChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getpinglunfenxiCount() {
			this.$http({
				url: `pinglunfenxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.pinglunfenxiCount = data.data
				}
			})
		},
		pinglunfenxiChat1(e=null) {
			this.$nextTick(()=>{
				var pinglunfenxiChart1 = echarts.init(document.getElementById("pinglunfenxiChart1"),'macarons');
				let params = {
				}
				this.$http({
					url: "pinglunfenxi/group/qingganfenxi",
					method: "get",
					params
				}).then(({ data }) => {
					if (data && data.code === 0) {
						...
						for(let i=0;i<res.length;i++){
							...
							xAxis.push(res[i].qingganfenxi);
							yAxis.push(parseFloat((res[i].total)));
							pArray.push({
								value: parseFloat((res[i].total)),
								name: res[i].qingganfenxi
							})
						}
						var option = {};
						...
						// 使用刚指定的配置项和数据显示图表。
						pinglunfenxiChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							pinglunfenxiChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
<style lang="scss" scoped>
	.home-content {
		...
		background: url(http://codegen.caihongy.cn/20250920/02c1a6846d354eaeacf14242828d10f4.png) no-repeat center top / cover;
		display: flex;
		width: 100%;
		min-height: 100vh;
		flex-wrap: wrap;
