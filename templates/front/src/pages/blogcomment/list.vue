<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item" >
					...
					<el-input v-model="formSearch.pluser" placeholder="评论人" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item" >
					<div class="lable">发布地：</div>
					<el-input v-model="formSearch.fbplace" placeholder="发布地" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('blogcomment','新增')" type="primary" @click="add('/index/blogcommentAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('blogcomment','发布地')" type="success" @click="chartDialogShow1">
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column :resizable='true' :sortable='false'
						...
						<template slot-scope="scope">
							<span @click="linkOthers(scope.row.detailurl)">{{scope.row.detailurl}}</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false'
						prop="pluser"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-btn1" type="success" @click.native="discussClick(scope.row)" v-if="btnAuth('blogcomment','查看评论')">
								<span class="icon iconfont icon-xiaoxi-zhihui"></span>
								查看评论
							</el-button>
							<el-button class="table-btn5" type="success" @click.native="chapterClick(scope.row)" v-if="btnAuth('blogcomment','章节管理')">
		...
		<el-dialog
			...
			width="800">
			<div id="fbplaceChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.pluser != '') searchWhere.pluser = '%' + this.formSearch.pluser + '%';
				if (this.formSearch.fbplace != '') searchWhere.fbplace = '%' + this.formSearch.fbplace + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`blogcomment/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/blogcommentDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("blogcomment/group/fbplace",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
