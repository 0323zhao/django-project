	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/blogcomment?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">评论人</div>
						<div class="text "  >{{detail.pluser}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">支持数</div>
						<div class="text "  >{{detail.likecount}}</div>
					</div>
					<div class="item">
						<div class="lable">发布地</div>
						<div class="text "  >{{detail.fbplace}}</div>
					</div>
					<div class="item">
						<div class="lable">评论时间</div>
						<div class="text "  >{{detail.pltime}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('blogcomment','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('blogcomment','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('blogcomment','评论分析')" 
							@click="onAcross('pinglunfenxi','','','','')" type="warning">
							评论分析
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('blogcomment','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('blogcomment','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
						...
						<div class="comment-item" v-for="item in infoList" :key="item.id" @mouseenter="discussEnter(item.id)" :id="'discuss' + item.id"
							...
							<div class="comment-content-box">
								...
								<div class="comment-btn">
									<!-- <el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#EABB3F","width":"auto","lineHeight":"34px","fontSize":"14px","height":"34px"}'>回复</el-button> -->
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`blogcomment`);
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('blogcomment/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'blogcomment', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			async thumbsupOrCrazily(type) {
				...
				await this.$http.post('storeup/add', this.storeupParams).then(res => {
					if (res.data.code == 0) {
						...
						this.$http.post('blogcomment/update', detail).then(res => {
							this.detail = detail
						});
						this.getThumbsupOrCrazilyStatus();
						this.$message({
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'blogcomment', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'blogcomment', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/blogcomment', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page,type=1) {
				this.$http.get('discussblogcomment/list', {
					params: {
						page, 
						limit: this.pageSize, 
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussblogcomment/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussblogcomment/delete',[id]).then(res=>{
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussblogcomment/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
						}
			...
			// 修改
			editClick(){
				this.$router.push(`/index/blogcommentAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此博文评论？') .then(_ => {
					this.$http.post('blogcomment/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterblogcomment', query: params});
			},
			discussClick (){
				let params = {
					refid: this.detail.id
				...
				this.$router.push({path: '/index/discussblogcomment', query: params});
			},
			// 获取uuid
			getUUID() {
