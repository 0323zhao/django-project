	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/bloginfo?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">博文</div>
						<div class="text "  >{{detail.mblogtext}}</div>
					</div>
					<div class="item">
						<div class="lable">评论数</div>
						<div class="text "  >{{detail.commentscount}}</div>
					</div>
					<div class="item">
						<div class="lable">转发数</div>
						<div class="text "  >{{detail.repostscount}}</div>
					</div>
					<div class="item">
						<div class="lable">点赞数</div>
						<div class="text "  >{{detail.attitudescount}}</div>
					</div>
					<div class="item">
						<div class="lable">发布时间</div>
						<div class="text "  >{{detail.fbtime}}</div>
					</div>
					<div class="item">
						<div class="lable">媒体</div>
						<div class="text "  >{{detail.medias}}</div>
					</div>
					<div class="item">
						<div class="lable">主页地址</div>
						<div class="text  link"  @click="linkOthers(detail.userurl)">{{detail.userurl}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('bloginfo','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('bloginfo','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('bloginfo','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('bloginfo','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.screenname;
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('bloginfo/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'bloginfo', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			async thumbsupOrCrazily(type) {
				...
				await this.$http.post('storeup/add', this.storeupParams).then(res => {
					if (res.data.code == 0) {
						...
						this.$http.post('bloginfo/update', detail).then(res => {
							this.detail = detail
						});
						this.getThumbsupOrCrazilyStatus();
						this.$message({
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'bloginfo', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'bloginfo', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/bloginfo', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page,type=1) {
				this.$http.get('discussbloginfo/list', {
					params: {
						page, 
						limit: this.pageSize, 
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussbloginfo/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussbloginfo/delete',[id]).then(res=>{
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussbloginfo/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
						}
			...
			// 修改
			editClick(){
				this.$router.push(`/index/bloginfoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此博文信息？') .then(_ => {
					this.$http.post('bloginfo/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
